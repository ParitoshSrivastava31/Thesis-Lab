...
export function runPropagation(
  ...
  while (queue.length > 0 && iterations < MAX_ITERATIONS) {
    ...
    for (const edge of outgoingEdges) {
      ...
      const decayFactor = edge.decayFactor !== undefined ? edge.decayFactor : 0.6; // Each hop loses 40% influence by default
      const effectMultiplier = edge.multiplier !== undefined ? edge.multiplier : getEffectMultiplier(edge.dependencyType);
